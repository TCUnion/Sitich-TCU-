...
自行車挑戰社群平台（TCU CHALLENGE）— Google Stitch 生成的前端介面，整合 Gemini AI。

## 會話規則

新對話開始時讀取 `/Volumes/OWC 2T/ClaudeCode/MyObsidian/brain/handoffs/Sitich-TCU小幫手.md`（先看「立即上手」即可上工）。
...
- **禁止**在根目錄建立非必要檔案
- **禁止**建立重複/版本化的檔案（`_v2`、`enhanced_`、`new_`）→ 擴充現有檔案
...
**功能**：挑戰賽瀏覽與報名、個人成績追蹤、車友社群與排行榜、AI 助理（Gemini API）
...
### Strava 認證流程（popup + postMessage，含 iOS fallback）
...
**iOS Safari fallback（window.opener = null）：**
```
使用者點擊登入
  → Safari 開新 Tab（非 popup）
  → Strava OAuth 授權
  → n8n callback 頁偵測 window.opener 為 null
  → redirect 到 strava.criterium.tw/#/auth?auth_token=...&athlete=...
  → useAuth.ts mount 時讀取 hash → 更新 state + 寫入 localStorage → 清除 hash
```
